...
LOWSN_STATUS_ENUM  usrRxPacketCallback(void) 
{
	...
	char *rcv;
        UINT8 recplen;
        UINT8 sizercv=0;
       char s1[]={0x66,0xAB,0x0B,0x01,0x16,0x99,0xfe,0x01,0x21,0x08,0x7f};
	char s2[]={0x66,0xAB,0x0D,0x01,0x31,0x00,0xFE,0x03,0x21,0x08,0x11,0x02,0x00};
	int flag1,flag2,flag3,i,j,k;
       static char load[]={0x00,0x11,0x22,0x33,0x44,0x55,0x44,0x33,0x22,0x11,0x00,0xaa};
	static char load1[]={0x66,0xAD,0x1C,0x01,0x31,0x00,0xFE,0x12,0x21,0x08,0x01,0x02,0x0F,0x60,0x82,0x1C,0x86,0x9B,0x3A,0x01,0x0A,0x00,0x0B,0x01,0x02,0x03,0x04,0x05};
	static char load2[]={0x66,0xAD,0x18,0x01,0x31,0x00,0xFE,0x12,0x21,0x08,0x11,0x02,0x0C,0x01,0x01,0xC9,0x00,0x07,0x04,0x30,0x00,0x20,0x0F,0x01};

	 a_aps_rx_data.usrPload=a_aps_rx_data.usrPload-5;
	 recplen=a_aps_rx_data.usrPlen+5;
...
//		conPrintROMString("Not a client/server type, discard it. ");
	...
	ResponseParam.actType = aplGetRxActType();
	 ...
	 //ResponseParam.plen =(BYTE)sizeof(load)/sizeof(load[0]);

	 for(i=0;i<recplen;i++)
	 {
	 	rcv[i]=*(a_aps_rx_data.usrPload);
	...
	{  
		...
		ResponseParam.pload = (BYTE *)(load1);
	 	ResponseParam.plen =(BYTE)sizeof(load1)/sizeof(load1[0]);	
	}
	else if(flag2==1)
		{
		ResponseParam.pload = (BYTE *)(load2);
	 	ResponseParam.plen =(BYTE)sizeof(load2)/sizeof(load2[0]);	
		}
	else 
		{
...
*/
/*	 if(*(a_aps_rx_data.usrPload+2)==0x0B)
	 {
	 	ResponseParam.pload = (BYTE *)(load1);
	 	ResponseParam.plen =(BYTE)sizeof(load1)/sizeof(load1[0]);	
	 }
	 else if(*(a_aps_rx_data.usrPload+2)==0x0D)
	 {
	 	ResponseParam.pload = (BYTE *)(load2);
	 	ResponseParam.plen =(BYTE)sizeof(load2)/sizeof(load2[0]);	
	 }
